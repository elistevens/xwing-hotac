...
class EnemyAi
    ...
    onClick_move: (event) ->
        ...
        $("##{@elem_id} .#{event.data.sector}.#{event.data.distance}.display i")\
                .addClass("xwing-miniatures-font xwing-miniatures-font-#{move_tup[0]}#{move_tup[1] or ''}")
        $("##{@elem_id} .#{event.data.sector}.#{event.data.distance}.display span").text(move_tup[2])
